...
import {
    ...
    DeviceCheckStateReplyData,
    DeviceStateChangedData,
    InterServerEvents,
    ServerToClientEvents,
    SocketData,
    UserCheckStateReplyData,
    UserCheckStateRequestData,
    UserCommandData,
} from 'hc_models/types';
...
export type HCGatewayServer = Server<
    ClientToServerEvents,
    ServerToClientEvents,
    InterServerEvents,
    SocketData
>;
export type HCGatewayNamespace = Namespace<
    ClientToServerEvents,
    ServerToClientEvents,
    InterServerEvents,
    SocketData
>;
export type HCGatewaySocket = Socket<
    ClientToServerEvents,
    ServerToClientEvents,
    InterServerEvents,
    SocketData
...
export class HCGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
    ...
    constructor(private readonly api: RepoService) { }

    async afterInit(server: Server) {
        server.use(authMiddleware(this.api, this.namespace, this.logger));
    }
    ...
    async handleCheckStateRequest(
        @ConnectedSocket() socket: HCGatewaySocket,
        @MessageBody() msg: UserCheckStateRequestData,
    ): Promise<UserCheckStateReplyData> {
        ...
        if (sockets.length === 0) {
            this.logger.debug(
                `Received userCheckStateReply from [user/${socket.data.userId}] for invalid/offline device`,
            );
            return { data: null };
        }
        ...
        if (deviceSocket.data.ownerId !== socket.data.userId) {
            this.logger.debug(
                `Received userCheckStateReply from [user/${socket.data.userId}] for a disallowed device`,
            );
            return { data: null };
        }
        ...
        return await new Promise(resolve => {
            ...
            socket.data.checkStateCallbackQueue?.push((data: object) => {
                if (!timedOut) {
                    ...
                    this.logger.debug(
                        `State check reply from [device/${deviceId}] received after timeout`,
                    );
                }
            });
            ...
            this.namespace()
                .in(`device_${deviceId}`)
                .emit('deviceCheckStateRequest', { socketId: socket.id });
        });
    }

    @SubscribeMessage('deviceCheckStateReply')
    async handleCheckStateReply(
        @ConnectedSocket() socket: HCGatewaySocket,
        @MessageBody() msg: DeviceCheckStateReplyData,
    ) {
        const sockets = await this.server.in(msg.socketId).fetchSockets();

        if (sockets.length !== 1) {
            this.logger.debug(
                'Received [deviceCheckStateReply] for bad socket ID, may have disconnected',
            );
            return;
        }
    ...
    async handleCommand(
        @ConnectedSocket() socket: HCGatewaySocket,
        @MessageBody() msg: UserCommandData,
    ) {
        const deviceId = msg.deviceId;
        ...
        if (sockets.length === 0) {
            this.logger.debug(
                `Received [userCommand] from [user/${socket.data.userId}] for invalid/offline device`,
            );
            return { data: null };
        }
        ...
        if (deviceSocket.data.ownerId !== socket.data.userId) {
            this.logger.debug(
                `Received [userCommand] from [user/${socket.data.userId}] for a disallowed device`,
            );
            return { data: null };
        }
    ...
    async handleStateChanged(
        @ConnectedSocket() socket: HCGatewaySocket,
        @MessageBody() msg: DeviceStateChangedData,
    ) {
    ...
    private async handleDeviceConnect(socket: HCGatewaySocket) {
        if (socket.data.deviceId && socket.data.ownerId) {
            ...
            this.namespace()
                .in(`user_${socket.data.ownerId}`)
                .emit('userDeviceConnected', { deviceId: socket.data.deviceId });
        }

        this.logger.verbose(
            `[device/${socket.data.deviceId}] owned by [user/${socket.data.ownerId}]`,
        );
    ...
    private async handleDeviceDisconnect(socket: HCGatewaySocket) {
        if (socket.data.deviceId && socket.data.ownerId) {
            this.namespace()
                .in(`user_${socket.data.ownerId}`)
                .emit('userDeviceDisconnected', { deviceId: socket.data.deviceId });
        }
